<html>
<head><title>kadai2-6</title></head>
<body>
...
function WriteData(){
	...
	$data = $num."<>".$name."<>".$contents."<>".$time."<>".$pass."\n";
...
function DeleteData(){
	...
	$list = file($keijiban_file);
	//パスワード認証
	$item = explode("<>", $list[$_POST['delete_num']-1]);
	if($_POST['password']==intval($item[4])){
		//処理部
		$data = null;
...
//標準のフォームを出力する関数
function PrintForm(){
	print  '<form action="kadai2-6.php" method="post">
				  名前 :<br />
...
function PrintFormEdit(){
	...
	//パスワード認証
	$item = explode("<>", $list[$_POST['edit_num']-1]);
	if($_POST['password']==intval($item[4])){
		foreach ($list as $key => $value) {
			$item = explode("<>", $value);
